...
@Component
public class ChangeSecretKeyRequestValidator  implements Validator<ChangeSecretKeyRequest> {

  public void validate(ChangeSecretKeyRequest request) {
    ...
    Validate.notEmpty(request.getPassword(), "The password should not be empty");

    Validate.notEmpty(request.getSecretKey(), "The secretKey should not be empty");
    Validate.isTrue(request.getSecretKey().length() <= 32,
                    "The length of secretKey can't exceed 32.");
